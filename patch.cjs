...
const replacement = `  renderLevels() {
      ...
      <main class="med-map-book-container">
        ...
        <div class="med-map-book">
          ...
          <div class="med-book-page right-page">
             ...
             <p class="med-region-desc">\${selectedLevel.summary || selectedLevel.subtitle}</p>
...
const start = code.indexOf('  renderLevels() {');
const end = code.indexOf('  renderDeckBuilder() {');

let modified = code.substring(0, start) + replacement.replace(/\\\\`/g, '`') + '\n\n' + code.substring(end);
