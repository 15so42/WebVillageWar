...
const CHANGELOG_ENTRIES = [
  ...
  },
  {
    ...
    title: '风法师与战斗表现优化',
    items: [
      '新增风法师单位与卡牌：每 7 秒向前唤出一道缓慢推进的飓风，每 0.4 秒对范围内敌人造成魔法伤害并将其吸引聚拢；配套「飓风续航」与「风暴回息」两项兵种专精，联机同步飓风特效。',
      '第一关 Boss 冰霜巨魔重做面部：凶悍的怒目眉脊、上翘外撇的长獠牙与头顶冰冠，去掉原本的憨厚轮廓。',
      '移动指令改为到达目标点后自动驻守，途中遇敌打完会继续前往，到达即转入驻守。',
      '移除手牌旁的临时牌位：战利品与调度出的牌直接排到抽牌堆顶，手牌有空位时立即抽入。',
      '建筑作用范围改为淡淡的圆形填充 + 更细的虚线边界，驻守环收细并降低亮度，减少对单位和战场的遮挡。',
      '野火火苗改回主世界层级并开启深度测试，会被单位与地形正常遮挡，不再浮在画面前方。',
      '雷云补上中心暗核让云块融合成整体，闪电更曲折细碎、辉光与亮核降亮，并在消散时释放独立材质避免泄漏。',
      '死亡烟雾的实心扩散环改为大小速度不一的低多边形烟块径向爆发，先快后慢外扩并淡出。'
    ]
  },
  {
    date: '2026-09-06',
    title: '雪原谷地雪面与营火表现优化',
    items: [
      '调整第一关开局构图与冷暖光影，统一雪地、岩壁和松林色调，改善近景与远景层次。',
      '雪地新增大小、方向和疏密不同的低矮雪脊与浅洼，形成与山体呼应的不规则折面；道路、营地与祭坛周围保留平缓过渡。',
      '修复营火火焰的白色片影与方形边缘，改善火苗消散和地面辉光；树木保持原布局并随雪面贴地。'
    ]
  },
  {
    date: '2026-09-05',
    title: '雪原谷地场景与光影重制',
    items: [
      '重做第一关高岩壁、覆雪台地与松林，调整木制瞭望塔和篝火布局，保留中央作战空间与原有相机视角。',
      '加强暖色斜阳与冷色背光岩壁的明暗对比，统一雪地、岩石与道路色调；道路改为灰褐色碎土，并加入不规则积雪路缘。',
      '树木按完整树冠避让岩壁、树木和场景物件，松树与木塔按真实雪面高度落地，减少穿插和悬空。'
    ]
  },
  {
    date: '2026-08-29',
    title: '附魔、建筑、Boss 与战斗特效大更新',
    items: [
      '审判附魔改为友方单位受击即触发（含持有者自身，各按独立 5 秒冷却），落剑大小与位置随目标体型缩放；烟花附魔重做为瞬发球形爆裂，范围 5 米内伤害与群体回血、6 秒冷却。',
      '重甲附魔护甲加成下调为每级 +0.5；燃烧/寒霜/出血附魔统一改名并补齐冰晶、烈焰等持续特效；波次奖励不再发放兵种专精卡牌。',
      '所有建筑费用统一为 4 能量、5 秒建成，并新增虚线作用范围环（箭塔射程/食堂治疗/维修站修复/信标部署）；矮人工匠现在可维修所有建筑（含基地），每次固定 5% 血量和耐久；基地瞭望改为每层 +50% 初始攻击距离（加算）。',
      '第一关新增两个特色 Boss：凛霜狼王（霜牙扑击位移群伤 + 狼群附魔持续召唤冰狼）与冰川先知（追逐最近敌人的冰霜风暴 + 冰镜结晶吸收伤害并反弹基地激光）；冰霜巨魔增加锤击溅射与风暴可见性。',
      '冲击环特效统一为渐变羽化 + 轻微扭曲的形态；回血粒子、buff 粒子（燃烧/毒/诅咒/寒霜等）放大并移入覆盖层，不再受描边影响也不被单位遮挡。',
      '雷云重做为多面体云块互相融合的连续云团（闪烁闪电保留）；炸弹爆炸特效完善为火球+冲击环+灰烟+崩落岩块；熔岩喷发改为多道橙黄光束向上喷发并伴随喷发粒子。',
      '死亡特效的冲击波恢复正常遮挡层级；野火地形火苗数量翻倍、更宽、根部锚定且绕过描边；白烟特效放大增厚。',
      '第一关 Boss（冰霜巨魔）改为石头傀儡风格并缩小 40%，锤子改用多面体并配身体同色系与尖刺。'
    ]
  },
  {
    date: '2026-08-28',
  ...
  },
  {
    ...
    items: [
      '第一关相机更新为完整 45° 俯视构图；渲染参数导出现在同时记录相机位置与观察目标，避免回填参数后视角偏移。',
      '两侧山体整体向战场中轴收拢，并将山体三角面材质改为共享顶点色批次，消除逐三角形绘制开销。',
      '雪谷主路移除重复高度扰动并压平核心截面，细化地面网格、降低路面碎石，解决道路横向褶皱。',
      '三座祭坛重新分布到道路两侧，移除第一关野怪、半透明地面贴片，以及敌营后方容易误认成目标的额外帐篷与围栏木寨。'
    ]
  },
  {
    date: '2026-08-21',
    title: '雪谷中尺度布景与地形深化',
    items: [
      '中央谷地由“整块平雪地”改为柔和浅谷：谷底略低、两侧山脚略高、温柔起伏，主路/基地/敌营/祭坛与战斗平台保持平坦可走。',
      '新增中尺度布景：道路两侧 6 组非镜像小景（中石+小松+雪堆）、道路边缘压雪/雪脊/冻土、祭坛半埋石环+木桩落位、路侧倒木锚点、低对比大色块雪地；战斗区保持留白。',
      '山体清理未接线的峡谷墙死代码；山体颜色去紫提亮，道路改浅灰卡其，外圈山改为大块并左右+前后补满全图（相机平移/前后移动不见空白）。',
      '第一关开局相机更新为 (-0.313, 37.396, 83.759)，山体整体外移配合回拉镜头。'
    ]
  },
  {
    date: '2026-08-20',
    title: '雪谷恢复白天暖阳渲染并新增峡谷山体层次',
    items: [
      '雪谷第一关恢复为白天「暖主光冷阴影」开阔雪谷：太阳为唯一暖光源、背光与阴影走冷蓝紫，树冠交回场景光照（绿色），夜间篝火方向整体撤销。',
      '峡谷改为山体网格实现：主路两侧新增两层低模山体墙（内低外高、错落留豁口），配中景山脊环与北侧/两翼雪峰远景，营造远近与高低层次；山体全部注册导航阻挡且离主路≥10m，主路/祭坛/基地保持可走。',
      '开局相机设为 (0.847,37.396,73.57)，目标 (0,4,18)。',
      '预览页隐藏操作提示条，避免入镜；祭坛取占环配色沿用未提交微调。'
    ]
  },
  {
    date: '2026-08-19',
    title: '雪谷战场开阔化重做',
    items: [
      '雪谷战场移除全部山体台地，改为开阔雪原：两翼与内侧散布低矮雪覆岩堆作掩体，两座岩堆顶立瞭望塔，主路走廊完全开阔。',
      '地图边缘新增一圈雪山远景轮廓（纯视觉、不参与导航阻挡），色彩向雾色靠拢做大气透视。',
      '雪谷光照改为暖主光冷阴影：太阳为唯一暖光源，背光面与阴影走冷蓝紫；岩石与远山三段色阶烘焙统一受光方向，树冠交回场景光照自动计算。',
      '雪谷关闭描边后处理，改由轮廓光表现边缘；松林通道、霜脊前线等其他地图保持原貌。',
      '清理旧场景规范文档：移除 AGENTS.md 中美术方向等章节与地形生成文档，场景设计重新从零开始。'
    ]
  },
  {
    date: '2026-08-14',
  ...
  },
  {
    ...
    title: '雪原第一关重做为开阔雪谷',
    items: [
      '雪原山体改为环形环抱布局：四角高崖、东西两翼中台地、南北低平台过渡，中央留出大片开阔雪谷；主路蛇形横穿谷地，去掉崖壁贴身夹路的峡谷压迫感。',
      '移除贯穿谷地的冰川河与小冰潭，谷地改为连续开阔雪面；主路调整为沿西翼下行、横穿谷地、沿东翼直取敌营。',
      '新增四角高地木质瞭望塔（雪帽顶与小红旗）与谷地三处帐篷火堆小营地，丰富雪谷的世界叙事。',
      '光照改为暮色冰河氛围：天空头顶深青蓝、地平线暖金，太阳抬高加暖，受光雪面读作明亮通透的白雪。',
      '远景新增三层雪岭山影带与山脊积雪层次，森林密度精简，开阔地边缘补充地标石阵与漂石点缀。'
    ]
  },
  {
    date: '2026-08-06',
    title: '翡翠沼泽场景与渲染优化',
    items: [
      '「翡翠沼泽」整体场景重构：新增 S 形主路、八个水塘与栈桥、远景枯林剪影层次，主路、基地与祭坛均保持干燥可通行。',
      '沼泽树改为组团种植：主树带头、伴生小树高低错落，林缘零星游离小树软化边界，不再是齐高的树墙。',
      '草地改为地形顶点色斑表现（亮绿疏草与暗绿密草噪声分区），移除俯视角读成碎块的 3D 草丛；地面补充碎石、倒木、低灌木与地被色斑。',
      '修复水面 shader 周期性闪光导致水面上出现间距相等的漂浮亮斑；移除俯视角成方片与光斑的贴水雾面和萤火粒子。',
      '沼泽渲染改为暖色主光与冷色阴影的昼夜氛围，调整雾距与背景色，水面保留深度渐变的动态波光。'
    ]
  },
  {
    date: '2026-08-06',
