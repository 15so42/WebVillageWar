...
- `UnitRegistry`：单位出生/死亡主动注册和注销。
- `UnitLogicSystem`：单位状态机、单位自己的计时器和 AI 决策。
- `TargetingSystem`：空间网格索敌，避免全场单位扫描。
- `PathfindingSystem` / `MovementAgent`：A* 寻路和单位沿路径移动。
- `AttackSystem`：攻击事件、延迟命中、投射物和弹体池。
- `CombatSystem`：只做伤害结算、闪避、护盾/生命扣除、击退、受击反馈和死亡触发，不写具体附魔名字。
...
- `recovery`：每秒恢复 `恢复等级 * 0.25` 的生命值。
- `spiritShield`：每秒获得 `灵盾等级 * 0.15` 的护盾，并通过 `maxShield` 加法修改器增加 `灵盾等级 * 0.5` 的最大护盾。
