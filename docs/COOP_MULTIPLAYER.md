...
更新日期：2026-07-24
状态：**规范 v2.3；多人联机网络同步已基本可用，未勾选项目仍需继续实机验收**
...
| 无尽难度、敌人交战时间评价 | 计算并在变化时同步难度值 | 只显示 Host 难度，不记录或计算评价 |
...
- 支持两个或更多玩家，不以双人为上限。
- 普通联机战役开局前，Host 合并全员提交的卡牌，并对每个 `cardDefinitionId` 取全员最高等级；所有玩家从同一套共享结果创建各自的运行时牌组。进入战斗后，手牌、抽牌堆、弃牌堆、能量、银币、能力、商店、奖励队列和选择状态仍按玩家隔离。
- 每位 PvE 玩家的能量由 Host 按每秒 `0.2` 独立恢复并同步；普通击杀不再提供基础能量，只有“猎魂潮汐”等明确的击杀能力可以额外获得能量。击杀银币仍按现有规则分别结算。
- 同一 PvE 队伍共享基地；共享基地死亡，队伍内所有玩家失败。
- 合作敌军按实际人数等额提高血量：双人 `2x`、三人 `3x`、四人及以上 `4x`；攻击分别为 `1.1x`、`1.2x`、`1.3x`，数量不变。缩放必须从 `gameData.js` 的模式配置读取，不得在网络代码里临时推导。
- 波次奖励为每个玩家分别生成和分发；同一波次不同玩家可以获得不同候选项。每次重新随机固定消耗该玩家 `4` 银币，由 Host 校验、扣除并同步私有状态。
...
- 房主在创建房间时锁定地图关卡、`challengeMode`（普通或无尽）与普通模式下自己已解锁的难度；无尽模式固定从难度 0 开始。每位玩家准备时提交自己的已拥有卡牌及等级。Host 开局时合并全员卡牌：普通战役逐卡取全员最高等级，无尽模式则在合并后按无尽规则统一重置为 Lv.1。玩家修改牌组、模式、阵营、地图、难度或会影响对局的配置后，Host 必须将相关玩家恢复为未准备。
...
- `ready` 命令携带 `gameVersion`、`deckRevision`、`catalogVersion` 和本玩家卡牌等级。Host 验证卡牌数量、拥有权、重复限制和版本后，才广播 `player_ready_changed`；共享牌组与最高等级只能由 Host 在开局事务中计算。
...
无尽模式由 Host 按需生成无限波次。进入战斗时所有玩家运行时牌组统一重置为 Lv.1，局外卡牌升级不生效且不修改存档；后续对局内升级照常应用。敌人第一次被非空攻击者实际造成伤害时，Host 记录该敌人的交战开始时间；死亡后的难度增减按“首次受伤到死亡”的交战时长与标准交战时长比较。标准交战时间按旧出生计时口径的 25% 计算，普通/精英/Boss 基准分别为 5 / 8.75 / 17.5 秒。Client 仅通过 `match` 状态补丁接收 `endlessDifficulty`，并在完整快照中恢复该值。任一基地被摧毁后由 Host 生成带 `endReason`、`endingDifficulty` 和个人权威金币的最终结算。
...
- [ ] 任一玩家准备后修改牌组会正确取消准备。
- [ ] 普通联机战役逐卡取全员最高等级，并给每位玩家生成内容与等级一致、运行时互不引用的牌组副本。
...
- [ ] 共享开局牌组生成后，每位玩家的运行时牌区、能量、银币、能力、商店和奖励互不覆盖。
