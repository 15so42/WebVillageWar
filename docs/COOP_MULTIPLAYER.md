...
更新日期：2026-07-24
状态：**规范 v2.3；双人联机网络同步已基本可用，未勾选项目仍需继续实机验收**
...
| 无尽难度、敌人存活时间评价 | 计算并在变化时同步难度值 | 只显示 Host 难度，不记录或计算评价 |
...
无尽模式由 Host 按需生成无限波次。进入战斗时双方运行时牌组统一重置为 Lv.1，局外卡牌升级不生效且不修改存档；后续对局内升级照常应用。每个波次敌人的出生时间、标准存活时间以及死亡后的难度增减只存在于 Host；Client 仅通过 `match` 状态补丁接收 `endlessDifficulty`，并在完整快照中恢复该值。任一基地被摧毁后由 Host 生成带 `endReason`、`endingDifficulty` 和个人权威金币的最终结算。
