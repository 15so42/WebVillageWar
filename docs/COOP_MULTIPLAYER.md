...
本阶段不支持 Host 回连、Host 进程闪退恢复或 Host 迁移。Host 与中继断开后，中继保留房间记录 60 秒仅用于租约清理和通知；该房间不再接受 Host 或普通 Client 回连，连续 60 秒后释放。因为中继不保存游戏世界，任何客户端都不能从中继恢复已经丢失的 Host 权威状态。
