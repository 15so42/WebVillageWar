...
// Use JS string replacement properly to avoid regex dotall greedy match bugs
const startMenu = code.indexOf('  renderMainMenu() {');
const endMenu = code.indexOf('  renderLevels() {');
const endLevels = code.indexOf('  renderDeckBuilder() {');

let modified = code.substring(0, startMenu) + newMenu + '\n\n' + newLevels + '\n\n' + code.substring(endLevels);
